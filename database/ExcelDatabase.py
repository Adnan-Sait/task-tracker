...
def getTaskId(projectId: int, taskName: str) -> int:
    taskId = None
    cursor = connection.cursor()
    cursor.execute(SQLQueries.TASK_ID_GET +
                   " WHERE \"projectId\"=%s AND lower(\"taskName\")=%s;", [projectId, taskName.lower()])
    queryResult = cursor.fetchone()
    if (queryResult):
...
def getProjectId(projectName: str) -> int:
    cursor = connection.cursor()
    cursor.execute(SQLQueries.PROJECT_ID_GET +
                   " WHERE lower(\"projectName\")=%s;", [projectName.lower()])
    queryResult = cursor.fetchone()
    if (queryResult):
