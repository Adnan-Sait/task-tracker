...
def readDailyActivitySheet(fileName: str, sheetNames: list):
    ...
    try:
        ...
        taskSheetData = excelService.readDataFromSheet(
            excelFile, sheetNames[0])
        overviewSheetData = excelService.readDataFromSheet(
            excelFile, sheetNames[1], 4)
        dateRecorded = excelService.getDateRecordedFromFileName(fileName)
        taskDataConsolidated = excelService.consolidateSheetData(
            dateRecorded, taskSheetData)
        print(taskDataConsolidated)
        dailyActivityList = excelService.populateDailyActivityObject(
            taskDataConsolidated)
        dayOverview = excelService.populateDayOverview(
            taskDataConsolidated, overviewSheetData)
        excelService.saveDailyActivityListAndDayOverview(
            dailyActivityList, dayOverview)
        try:
            excelFile.close()
